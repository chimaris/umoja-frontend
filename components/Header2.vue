			...
			<div class="d-flex justify-space-between align-center">
				<div class="d-flex align-center">
					<v-avatar class="mr-8" @click="$router.push('/home2')" size="102" height="" style="cursor: pointer; height: 65px !important" rounded="0">
						<v-img eager :src="getCloudinaryImageUrl('https://res.cloudinary.com/dkbt6at26/image/upload/v1684229324/Frame_4_emeelq.png', 102)"></v-img>
					</v-avatar>
				...
				<div class="d-flex">
									...
									<v-slide-x-reverse-transition leave-absolute origin="center center">
										<v-btn v-if="!isActive" icon size="40" v-bind="props" rounded="xl" flat color="transparent" class="">
											<v-avatar rounded="0" size="20">
												<v-img
													contain
													:src="getCloudinaryImageUrl('https://res.cloudinary.com/payhospi/image/upload/v1691742708/outline-search-minimalistic-magnifer_eez0ab.png', 20)"
												></v-img> </v-avatar
										></v-btn>
					...
					<v-btn rounded="xl" v-if="!isLoggedIn" to="/signin-category" size="40" icon flat color="transparent">
						<v-avatar size="20"> <v-img :src="getCloudinaryImageUrl('https://res.cloudinary.com/payhospi/image/upload/v1693034207/User_Rounded_xfgovv.png', 20)"></v-img> </v-avatar
					></v-btn>
					<v-btn v-else rounded="xl" to="/user/profile/Account Settings" size="40" icon flat color="transparent">
						<v-avatar size="20"> <v-img :src="getCloudinaryImageUrl('https://res.cloudinary.com/payhospi/image/upload/v1689338074/frame-481584_vquap5.png', 20)"></v-img> </v-avatar
					></v-btn>
					<v-btn @click="toCart()" class="text-none" icon size="40" rounded="xl" flat color="transparent">
						<v-badge offset-y="20" :dot="cartStore.totalCartItems == 0" :content="cartStore.totalCartItems" color="error">
							<v-avatar rounded="0" size="20">
								<v-img contain :src="getCloudinaryImageUrl('https://res.cloudinary.com/payhospi/image/upload/v1691742709/outline-shopping-ecommerce-bag-4_aibbld.png', 20)"></v-img>
							</v-avatar>
						</v-badge>
					</v-btn>
						...
						<template v-slot:activator="{ props }">
							<v-btn icon size="40" v-bind="props" rounded="xl" flat color="transparent">
								<v-avatar rounded="0" size="16">
									<v-img
										contain
										:src="getCloudinaryImageUrl('https://res.cloudinary.com/payhospi/image/upload/v1691742990/outline-notifications-vector_vkwg9h.svg', 16)"
									></v-img> </v-avatar
							></v-btn>
						</template>
						<dialognotification />
					...
					<v-btn v-if="$vuetify.display.mobile" rounded="xl" size="40" icon flat color="transparent" @click.stop="openNav">
						<v-avatar size="20"> <v-img contain :src="getCloudinaryImageUrl('https://res.cloudinary.com/payhospi/image/upload/v1713788195/umoja/menu.svg', 20)"></v-img> </v-avatar
					></v-btn>
				</div>
			</div>
		</v-container>
