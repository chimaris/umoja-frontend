...
letter-spacing: -0.14px;"><span class="mr-1">{{'By '}}</span> <strong> {{ item.author }}</strong> <v-icon icon="mdi mdi-circle" class="mx-2" size="5" color="#CECECE"></v-icon> {{ item.date }}</p>
         <p  style="font-weight: 600;
    font-size: 24px;
    line-height: 140%;cursor: pointer;
    color: #1E1E1E;
    " class="  text-wrap">{{item.title}}</p>
    <p style="font-weight: 500;
    font-size: 16px;
    line-height: 140%;
    color: #333;
    " class="mt-1 text-wrap">Lorem ipsum dolor sit amet consectetur. Nibh vitae facilisi suspendisse tempor non.</p>
<div class="d-flex align-center mt-4">

    <v-chip :color="item.color" class="d-flex justify-center align-center" variant="tonal" size="x-small"  style="font-size: 10px;
font-weight: 600;border-radius: 6px;width: 97px;text-align: center;
letter-spacing: 0.3px;">{{item.category}}</v-chip>

<v-icon icon="mdi mdi-circle" size="5" class="mx-2" color="#CECECE"></v-icon> <span style="color#333;
font-size: 14px;
font-weight: 400;
