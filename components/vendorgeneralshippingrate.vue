			...
			<v-card class="my-5 pa-6 cardStyle" flat style="background-color: #f8f8f8; border: 1px solid #cecece">
				...
				<v-card v-if="shippingZones.length > 0" v-for="zone in shippingZones" :key="zone" class="mx-auto my-5 pa-6 cardStyle" flat style="justify-content: between">
							...
							<div class="d-flex">
								...
								<div>
									...
									<p style="font-size: 16px; font-weight: 500; color: #969696">
										<template v-if="!zone.showAll && zone.countries.length  > 3">
											{{ zone.countries.slice(0, 3).join(', ') }} ... 
											<span style="color: #1273eb; cursor: pointer; white-space: nowrap" @click="zone.showAll = true">Show all</span>
										</template>
										<template v-else>
											{{ zone.countries.join(', ') }} 
											<span v-if="zone.countries.length  > 3" style="color: #1273eb; cursor: pointer; white-space: nowrap" @click="zone.showAll = false">Show less</span>
										</template>
									</p>
								</div>
							</div>
							<div class="d-flex">
								<v-menu>
									...
									<v-card class="bg-white d-flex flex-column text-left pa-4 h-auto">
										...
										<span @click="deleteZone(zone.id)" class="pa-2 text-left d-flex">
											<div class="mr-4" style="width: 20px; height: 20px">
												<v-img
					...
					<v-row class="mb-1" v-for="rate in zone.rates" :key="rate" style="font-size: 16px; font-weight: 400; color: #333">
						...
						<v-col cols="3">
							<p class="text-truncate" style="max-width: 200px">{{rate.condition}}</p>
						</v-col>
						<v-col>
...
async function addZoneRate(){
	...
	if (!zoneId.value || !rateName.value || !customName.value || !ratePrice.value){
		return
	}
	try{
		loading.value = true
...
async function saveZone (){
	...
	try{
		...
		if (error.response) {
            zoneError.value = error.response.data.message || 'An error occurred while adding rate.';
          } else if (error.request) {
