<template>
    <div>
        <p 
            style="color: #000;
            font-size: 20px;
            font-weight: 600;" class="pa-5">Account Details</p>
            <v-divider></v-divider>
            <div class="pa-5 py-8">

<div class="mb-10 d-flex align-center justify-space-between">
<p style="font-size: 20px;
font-weight: 600;">Your bank account details </p>

<div>

    <v-btn @click="bankDetails = true" style="    border: 1px solid #e5e5e5;
...
font-size: 14px;
font-weight: 500;">Account Name</p>
<div class="d-flex  align-center">
    <p style="color: #000;
font-size: 14px;
font-weight: 500; text-transform: capitalize
">{{ vendor.vendor_details?.name_on_account }}</p>
<v-icon @click="copyToClipboard(vendor.vendor_details?.name_on_account)" color="green" size="small" class="ml-3" icon="mdi mdi-content-copy"></v-icon>
</div>
</div>
<div  class="d-flex   mb-8 justify-space-between align-center">
<p style="color: #969696;
font-size: 14px;
font-weight: 500;">Account Number</p>
<div class="d-flex  align-center">
    <p style="color: #000;
font-size: 14px;
font-weight: 500;
">{{vendor.vendor_details?.bank_account_number}}</p>
<v-icon @click="copyToClipboard(vendor.vendor_details?.bank_account_number)" color="green" size="small" class="ml-3" icon="mdi mdi-content-copy"></v-icon>
</div>
</div>
<div  class="d-flex   mb-8 justify-space-between align-center">
<p style="color: #969696;
font-size: 14px;
font-weight: 500;">Sort Code</p>
<div class="d-flex  align-center">
    <p style="color: #000;
font-size: 14px;
font-weight: 500;
				...
				<v-form v-model="valid" @submit.prevent="editBankDets()">
					<v-row>
						<v-col cols="12" class="pt-0" md="6">
							...
							<v-text-field v-model="vendor.vendor_details.name_on_account" placeholder="Enter your account name" density="comfortable"> </v-text-field>
						</v-col>
						<v-col cols="12" md="6" class="pt-0">
							<p class="inputLabel">Account Number<span class="mb-2">*</span></p>
							<v-text-field v-model="vendor.vendor_details.bank_account_number" placeholder="Enter your account number" density="comfortable"> </v-text-field>
						</v-col>
					</v-row>
                    <v-row>
						<v-col cols="12" class="pt-0" md="6">
		...
		<v-sheet max-width="949" class="pa-4 pa-md-6">
			...
			<v-card flat class="mt-6 bg-white rounded-lg py-8">
				<v-form v-model="valid" @submit.prevent="editBankDets()">
					...
					<p style="color: red; font-size: 16px">{{ bankError }}</p>
					<v-btn class="textClass px-8" rounded="xl" :disabled="status" type="submit" color="green" flat>{{ status? 'Adding': 'Add Account' }}</v-btn>
...
const copyToClipboard = (refName) => {
	...
	if (!props.vendor.vendor_details?.address || !props.vendor.vendor_details?.bank_name || !props.vendor.vendor_details?.bank_account_number || !props.vendor.vendor_details?.name_on_account){
		return
	}
	const data = {
