<template>
	<v-container height="100%" class="mx-auto px-5" width="100%" style="overflow: hidden; padding-bottom: 200px; max-width: 1330px" flat>
				...
				<div class="d-flex align-center">
					...
					>
						<template v-slot:append-inner>
							<v-progress-circular v-if="loading" color="grey" :size="24" :width="3" indeterminate></v-progress-circular>
						</template>
					</v-text-field>
				</div>
				<div class="d-flex align-center">
		...
		<div class="mt-5">
			...
			<v-table v-if="filteredProductsData.length > 0 && !productLoading" style="height: 80% !important; overflow: scroll">
				<thead>
				...
				<tbody>
					...
					<tr :style="chosen == item.id ? 'background:#DFDFDF' : ''" v-for="item in filteredProductsData" :key="item.id">
									...
									<template v-slot:activator="{ props }">
										<div v-bind="props" class="d-flex align-center pr-4 pl-2">
											<v-avatar color="grey-lighten-4" class="rounded-lg mr-3 ml-0" size="50"
												><v-img :src="item.photo.split(',')[0]" cover></v-img
											></v-avatar>
											<div>
												<p
									...
									<v-card max-width="310" class="pa-6 rounded-lg" color="#2C6E63">
										<v-row class="d-flex align-start">
											<v-avatar color="grey-lighten-4" size="70" rounded="lg">
												<v-img :src="item.photo.split(',')[1]" cover></v-img>
											</v-avatar>
											<v-col class="py-0">
												...
												<p style="color: #edf0ef; font-size: 16px; font-weight: 600; line-height: 20px" class="mt-1">
													{{ formattedPrice(item.price) }} per {{ item.unit }}
												</p>
											</v-col>
										</v-row>
									</v-card>
						...
						<td class="tableLight px-1">
							<p style="color: #333; font-size: 14px; font-style: normal; font-weight: 600">{{ item.sku }}</p>
						</td>
           ...
           <v-btn v-else color="green" size="small" class=" rounded-lg"> See details</v-btn>
				...
				<v-dialog persistent v-model="showDeleteModal" max-width="400">
					<v-card>
						...
						<v-card-text> This product will be archived and after 30 days it will be deleted. </v-card-text>
				...
				<v-dialog persistent v-model="showRestore" max-width="400">
					<v-card>
						...
						<v-card-text> This product will be restored and removed from archive. </v-card-text>
						<v-card-actions style="display: flex; justify-content: flex-end">
				...
				<v-dialog persistent v-model="vendorProducts.successArchive" max-width="400">
					<v-card>
						<v-card-text> Product has been Archived Successfully </v-card-text>
						<v-card-actions style="display: flex; justify-content: flex-end">
				...
				<v-dialog persistent v-model="vendorProducts.successRestore" max-width="400">
					<v-card>
						<v-card-text> Product has been restored Successfully </v-card-text>
						<v-card-actions style="display: flex; justify-content: flex-end">
				...
				<v-dialog persistent v-model="vendorProducts.successDelete" max-width="400">
					<v-card>
						<v-card-text> Product has been deleted permanently </v-card-text>
						<v-card-actions style="display: flex; justify-content: flex-end">
			...
			<div v-if="!hasProduct" class="d-flex flex-column justify-center align-center" style="max-height: 100%; height: 90vh">
				<v-sheet class="d-flex flex-column justify-center align-center text-center" style="width: 450px">
					...
					<div class="mt-10">
						<v-btn
							...
							size="x-large"
							width="191"
							style="border: 1px solid #cecece; padding: 12px 20px"
							flat
							@click="choose('Import Product')"
						>
							<v-icon class="mr-2" icon="mdi mdi-tray-arrow-down"></v-icon>
			...
			>
				<div style="display: flex; align-items: center">
					<span style="font-size: 14px; font-weight: 400"
						>{{ vendorProducts.from }} - {{ vendorProducts.toPage }} of {{ vendorProducts.pagesNo }} Pages</span
					>
				</div>
				<div class="d-flex align-center" style="margin-left: auto">
					...
					<v-btn :disabled="vendorProducts.currentPage == vendorProducts.pagesNo" @click="selectedPage++" flat
					...
					<div class="ml-5" style="flex: 1">
						...
						<p style="font-size: 18px">
							...
							<span style="color: #2c6e63"
								>{{ vendorProducts.newProductName }} <span>{{ vendorProducts.newProductsku }}</span></span
							>
						</p>
					</div>
