	...
	<v-col cols="12" lg="4">
		<v-card v-if="cartStore.checkoutItems.length > 0" style="border-radius: 15px; border: 1px solid var(--carbon-2, #cecece)" flat class="pa-4">
			<v-row dense v-if="!viewAll">
				...
				<v-col cols="5">
					<div class="pl-4 pr-4 pr-md-2" style="max-width: 250px; overflow: hidden; text-overflow: ellipsis">
						...
						<p style="font-weight: 500; font-size: 14px; line-height: 18px; color: #969696" class="text-truncate">
							Category: {{ cartStore.checkoutItems[0].category_name }}
						</p>
			...
			<div class="d-flex w-100 align-center">
				<v-text-field v-model="discountCode" hide-details="" placeholder="Add discount code" density="comfortable"> </v-text-field>
				<v-btn
					@click="cartStore.applyDiscountCode(discountCode)"
					flat
					size="large"
					...
					rounded="xl"
				>
					<span style="color: #333; font-size: 14px; font-weight: 600; margin-right: 5px"> Apply </span>
			...
			<p style="font-weight: 400; font-size: 14px; line-height: 24px" class="mt-4">
				<strong> New Customer? </strong>

				<a target="_blank" style="color: #0076ff; text-decoration: underline" href="#" v-bind="props" @click.stop> Sign up </a>
			...
			<div class="mb-3">
				<div class="d-flex pb-3 align-center justify-space-between">
					...
					<p style="color: var(--carbon-4, #333); font-size: 16px; font-weight: 600" class="">{{ formattedPrice(cartStore.checkoutTotalCost) }}</p>
				</div>
				<div class="d-flex pb-3 align-center justify-space-between">
