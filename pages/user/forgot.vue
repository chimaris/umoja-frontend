	...
	<div class="authBg">
		...
		<div class="d-flex justify-center py-12 px-2">
			<v-card flat class="pa-12 rounded-lg" width="100%" max-width="562px">
				...
				<v-text-field :rules="emailRules" placeholder="Enter email address" v-model="email" density="comfortable"> </v-text-field>
				<p color="red">{{ userStore.error }}</p>
				<v-row>
					<v-col cols="6">
					...
					>
					<v-col cols="6">
						<v-btn to="/user/login" block variant="outlined" flat size="x-large" class="rounded-lg ml-1 mt-6">
...
<script setup>
	...
	async function handleReset() {
		if (email.value) {
			try {
				const linkSent = await userStore.forgotPassword(email.value)
				if (linkSent) {
