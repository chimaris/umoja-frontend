								...
								<v-card flat class="pa-4 cardStyle rounded-lg justify-space-between align-center my-4 d-flex">
									<div class="align-center d-flex">
										<input
											...
											class="mr-2"
											style="accent-color: #2c6e63; transform: scale(2)"
										/>
										<label :for="'address_' + index" class="text-capitalize px-4">
											<p style="" class="addressName mb-2">{{ item.full_name }}</p>
											<p class="adddressPhone mb-1">
												{{ item.shipping_address }}, {{ item.shipping_city }}, {{ item.shipping_region }} {{ item.shipping_country }}
											</p>
											<p class="adddressPhone">{{ item.phone_number }}</p>
										</label>
									</div>
									<v-spacer></v-spacer>
									<v-btn @click="toggleEditAddress(index)" size="small" class="smallText" variant="text" color="blue"
										>Edit Address <v-icon class="ml-2" icon="mdi mdi-pencil-outline"></v-icon
									></v-btn>
								</v-card>
								<v-form @submit.prevent="updateShippingAddress(item)" v-if="editAddressIndex === index" @keydown.enter.prevent="" class="pb-5">
									...
									<v-row class="mt-6">
										<v-col class="pb-0" cols="12">
											...
											<v-text-field v-model="item.full_name" placeholder="Enter your full name" :rules="inputRules" density="comfortable">
											</v-text-field>
										</v-col>
									</v-row>
									<v-row>
									...
									<v-text-field v-model="item.shipping_postal_code" placeholder="Enter your zipcode" density="comfortable"> </v-text-field>
						...
						<v-card flat class="cardStyle bg-white rounded-lg mb-4 pa-4 py-6">
							...
							<v-form @submit.prevent="addShippingAddress" @keydown.enter.prevent="" v-if="addAddress">
								...
								<v-row class="mt-6">
									<v-col class="pb-0" cols="12">
										<p class="inputLabel">Full Name<span class="mb-2">*</span></p>
										<v-text-field v-model="fullName" :rules="inputRules" placeholder="Enter your full name" density="comfortable"> </v-text-field>
									</v-col>
								</v-row>
								<v-row>
									<v-col cols="12" md="6">
								...
								<p class="inputLabel">Street Name and House Number*<span class="mb-2">*</span></p>
								<v-text-field placeholder="Enter your Street Name and House Number" v-model="streetName" :rules="inputRules" density="comfortable">
								</v-text-field>
								<v-row class="">
									<v-col cols="12" md="6">
										<p class="inputLabel">State<span class="mb-2">*</span></p>
								...
								<p class="inputLabel">Postal code<span class="mb-2">*</span></p>
								<v-text-field v-model="postalCode" placeholder="Enter your zipcode" density="comfortable"> </v-text-field>
								<p style="color: red; font-size: 16px">{{ addressError }}</p>
								<v-btn type="submit" class="textClass px-8" rounded="xl" color="green" flat>
...
export default {
	...
	data() {
		return {
			...
			confirmEmail: [(v) => !!v || "Confirm your email address", (v) => this.email == v || "Must be the same as email address"],
	...
	methods: {
		...
		async addShippingAddress() {
			if (this.fullName && this.email && this.confirmationEmail && this.phoneNumber && this.streetName && this.shippingState && this.shippingCity) {
				const data = {
